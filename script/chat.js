...
module.exports.run = async function({ api, event, args }) {
	...
	if (isAdmin) {
		if (args[0] === "off") { // Changed "target" to "args"
			...
			return api.sendMessage(`🛡️ | Chat off has been activated.\n\n The bot will now remove non-admin members from the group when they chat.`, event.threadID);
		} else if (args[0] === "on") { // Changed "target" to "args"
			chat[String(event.threadID)] = false;
			writeFileSync(path, JSON.stringify(chat), 'utf-8');
			return api.sendMessage(`✅  | Chat off has been deactivated.\n\nThe bot will no longer remove members when they chat.`, event.threadID);
		} else {
